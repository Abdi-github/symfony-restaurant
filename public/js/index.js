...
$(function () {
  ...
  if (jQuery(".mobile_header").length > 0) {
    jQuery(".mobile_header").after(
      '<div class="mobile_menu_wrapper"><ul class="mobile_menu"/></div>'
    );
    jQuery(".mobile_menu").html(
      jQuery(".vincent_menu_cont").find("ul.vincent_menu").html()
    );
  ...
  function vincent_setup_box() {
    var swipe_slider = jQuery("#swipebox-slider"),
      ...
      setHeight = 0;
    setHeight =
      jQuery(window).height() - swipe_title.height() - swipe_bottom.height();
  ...
  if (jQuery(".vincent_single_album_head").length > 0) {
    ...
    jQuery("a.vincent_album_down_arrow").on("click", function () {
      ...
      jQuery("html, body")
        ...
        .animate(
          {
            scrollTop: vincent_album_featured_height + "px",
          },
          600
        );
    });
  ...
  jQuery(".vincent_back_to_top").attr(
    "data-bottom",
    parseInt(jQuery(".vincent_back_to_top").css("bottom"), 10)
  );
  if (vincent_window.scrollTop() > vincent_window.height()) {
    ...
    if (jQuery(".vincent_footer").length > 0) {
      var footer_offset = jQuery(".vincent_footer").offset().top,
        check_footer_state =
          vincent_window.scrollTop() + vincent_window.height(),
      ...
      jQuery(".vincent_back_to_top").css(
        "bottom",
        parseInt(jQuery(".vincent_back_to_top").attr("data-bottom"), 10) +
          vincent_b2t_fixer +
          "px"
      );
    }
...
jQuery(window).resize(function () {
  ...
  if (jQuery(".vincent_blog_grid_ratio").length > 0) {
    jQuery(".vincent_blog_grid_ratio").each(function () {
      var setHeight = Math.floor(
        jQuery(this).width() * jQuery(this).attr("data-ratio")
      );
...
function vincent_404_page_centered() {
  ...
  if (inner_container_404_height < container_404_height) {
    ...
    jQuery(".vincent_404_content_wrapper").css({
      "padding-top": white_space / 2,
      "padding-bottom": white_space / 2,
    });
  }
}
